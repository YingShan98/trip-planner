-- Lets each itinerary item carry its own opening hours, closed days, and
-- recommended weekdays — the kind of time-sensitive planning notes (avoid
-- Tuesday closures, prefer weekdays over weekends) that don't fit the
-- existing free-text notes without getting lost among everything else.

alter table public.activities add column if not exists visit_hours text not null default '';
alter table public.activities add column if not exists closed_days text not null default '';
alter table public.activities add column if not exists recommended_weekdays text not null default '';
