-- Optimistic concurrency control for trip content saves.
--
-- save_trip_workspace previously replaced the entire content of a trip (days,
-- checklist, hotels, ...) on every call with no way to detect that someone
-- else had saved in between: two people editing the same trip concurrently
-- meant whichever save landed last silently discarded the other's work.
--
-- trips.updated_at can't be used as the version token for this check because
-- it's also bumped by unrelated trip-metadata writes (SettingsModal's direct
-- `trips` update, set_trip_edit_password) that have nothing to do with
-- content conflicts. content_version is a dedicated counter that only
-- save_trip_workspace increments.

alter table public.trips add column if not exists content_version bigint not null default 0;
