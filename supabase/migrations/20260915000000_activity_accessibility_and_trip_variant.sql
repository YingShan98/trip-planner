-- Lets each itinerary item carry accessibility/mobility notes, a suggested
-- duration, a fallback alternative, and an early-exit option — the kind of
-- planning detail needed when a trip has to work for both energetic and
-- less-mobile travelers.
--
-- Also lets a trip carry a variant label + target audience (e.g. "relaxed" /
-- "年长人士／行动不便人士"), so a group that needs two different itineraries
-- for the same window (e.g. one for parents, one for the young crowd) can
-- tell the two trips apart at a glance in the trip list and on the trip page.
