-- Lets a note be attached to one hotel/day card (a discussion thread) instead of only the general notes wall.
-- Non-destructive: existing notes get target_type/target_index = null, meaning "general note", unchanged behavior.
