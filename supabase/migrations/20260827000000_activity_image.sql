-- Lets each itinerary item carry an optional photo of the place, shown as a
-- preview under the activity card (edit mode shows a URL field, view mode
-- shows the image when set).

alter table public.activities add column if not exists image_url text not null default '';
